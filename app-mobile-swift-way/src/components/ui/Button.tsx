...
interface ButtonProps extends TouchableOpacityProps {
  // aceita title (legado) ou children (padrão React) — um dos dois é suficiente
  title?: string;
...
export function Button({
  ...
  // label: children tem prioridade sobre title
...
const getSizeStyles = (size: 'small' | 'sm' | 'md' | 'lg') => {
  switch (size) {
    case 'small':
    case 'sm':
      return {
...
const getVariantStyle = (variant: string): ViewStyle => {
  switch (variant) {
    case 'secondary': return { backgroundColor: colors.card };
...
const styles = StyleSheet.create({
  ...
  textWithLeftIcon:  { marginLeft:  spacing.sm },
  textWithRightIcon: { marginRight: spacing.sm },
